...
def main():
    parser = argparse.ArgumentParser(description="Run default optimizer baselines across seeds.")
    ...
    for optimizer in config["optimizers"]:
        params = config["defaults"][optimizer]
    ...
    if summaries:
        write_json(output_root / "default_summaries.json", summaries)
        write_csv(output_root / "default_summaries.csv", summaries)
        print(f"saved={output_root / 'default_summaries.csv'}")
